...
# Parse arguments: random seed, number of points to sample,
#                  number of resamplings in bootstrap analysis
# Initial tests used Npts=100,000
# www.itl.nist.gov/div898/handbook/eda/section3/bootplot.htm
# --> Suggests ~500 resamples may be optimal
if len(sys.argv) < 2:
  print("Usage:", str(sys.argv[0]), "<RNG seed> <N_samples> <N_bootstrap>")
  sys.exit(1)
seed = int(sys.argv[1])
Npts = int(sys.argv[2])
...
for i in range(Nsamples):
  ...
  indices = prng.integers(0, high=Npts, size=Npts)
  for j in range(Npts):
